...
UNITS = "|".join(
    re.escape(u) for u in sorted({u for t in CATALOG for u in t.factors}, key=len, reverse=True)
)
FLAG = r"low|high|normal|hgh|h|l|n"
PATTERN = re.compile(
    rf"\b(?P<name>{ALIASES})\s*[:=]?\s*(?P<value>{NUMBER})\s*"
    rf"(?P<unit>{UNITS})(?![a-z0-9/^])"
    rf"(?:\s*(?:\((?P<flag1>{FLAG})\)|(?P<flag2>{FLAG})\b))?"
    rf"(?:\s*(?:(?:ref(?:erence)?(?:[ _-]?range)?|range)\s*[:=]?\s*|\[|\()?"
...
def parse(text: str) -> tuple[list[NormalizedTest], list[Evidence], str]:
    ...
    for match in matches:
        ...
        factor = definition.factors.get(raw["unit"].lower())
        if factor is None:
    ...
    remainder = re.sub(r"\b(?:CBC|complete blood count)\b", "", "".join(remaining), flags=re.I)
